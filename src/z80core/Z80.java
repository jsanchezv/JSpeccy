...
public class Z80 {
    ...
    public final int getRegR() {
        return (regR & 0x7f) | (regRbit7 ? SIGN_MASK : 0x00);
    }

    public final void setRegR(int valor) {
        regR = valor & 0x7f;
        regRbit7 = (valor & SIGN_MASK) != 0;
    }
    ...
    private final int pop() {
        int lsb = MemIoImpl.peek8(regSP);
        regSP++;
        int msb = MemIoImpl.peek8(regSP);
    ...
    private final void ini() {
        tEstados += MemIoImpl.MREQstates((regI<<8)|getRegR(), 1);
        ...
        if( (((regC + 1) & 0xff) + work8) > 0xff ) {
            sz5h3pnFlags |= HALFCARRY_MASK;
            carryFlag = true;
        }

        if( (sz53pn_addTable[(((((regC + 1) & 0xff) + work8) & 0x07) ^ regB)] &
    ...
    private final void nmi() {
        ...
        ffIFF2 = ffIFF1;
        ffIFF1 = false;
        push(regPC);  // +6 t-estados + contended si procede
        regPC = 0x0066;
    }
     ...
     * casi 7900 bytecodes.
     */
    public final int execInstruction() {
        int work8, work16;
        byte salto;
        ...
        switch (opCode) {
            ...
            case 0x03:       /*INC BC*/
                incRegBC();
                tEstados += MemIoImpl.MREQstates((regI<<8)|getRegR(), 2);
                break;
            case 0x04:       /*INC B*/
                regB = inc8(regB);
                break;
            ...
            case 0x09:       /*ADD HL,BC*/
                ...
                tEstados += MemIoImpl.MREQstates((regI<<8)|getRegR(), 7);
                break;
            case 0x0A:       /*LD A,(BC)*/
                regA = MemIoImpl.peek8(getRegBC());
                break;
            case 0x0B:       /*DEC BC*/
                decRegBC();
                tEstados += MemIoImpl.MREQstates((regI<<8)|getRegR(), 2);
            ...
            case 0x22: {     /*LD (nn),HL*/
                MemIoImpl.poke16(MemIoImpl.peek16(regPC), getRegHL());
                regPC += 2;
                break;
            }
            case 0x23: {     /*INC HL*/
            ...
            case 0x32: {     /*LD (nn),A*/
                MemIoImpl.poke8(MemIoImpl.peek16(regPC), regA);
                regPC += 2;
                break;
            }
            case 0x33: {     /*INC SP*/
            ...
            case 0x3A: {     /*LD A,(nn)*/
                regA = MemIoImpl.peek8(MemIoImpl.peek16(regPC));
                regPC += 2;
                break;
            }
            case 0x3B: {     /*DEC SP*/
            ...
            case 0xE3:       /*EX (SP),HL*/
                ...
                MemIoImpl.poke8(regSP + 1, (work16 >>> 8));
     ...
     */
    private final void decodeDDFD(IndexRegister regIXY) {
        int work8, work16, address;
        byte offset;
        regR++;
        opCode = MemIoImpl.getOpcode(regPC);
        regPC++;
        switch (opCode) {
            case 0x09: {     /* ADD IX,BC */
                regIXY.setWord(add16(regIXY.getWord(), getRegBC()));
                tEstados += MemIoImpl.MREQstates((regI<<8)|getRegR(), 7);
                break;
            }
            case 0x19: {     /* ADD IX,DE */
                regIXY.setWord(add16(regIXY.getWord(), getRegDE()));
                tEstados += MemIoImpl.MREQstates((regI<<8)|getRegR(), 7);
                break;
            }
            case 0x21: {     /*LD IX,nn*/
                regIXY.setWord(MemIoImpl.peek16(regPC));
                regPC = ((regPC + 2) & 0xffff);
                break;
            }
            case 0x22: {     /*LD (nn),IX*/
                work16 = MemIoImpl.peek16(regPC);
                MemIoImpl.poke16(work16, regIXY.getWord());
                regPC = ((regPC + 2) & 0xffff);
                break;
            }
            case 0x23: {     /*INC IX*/
            ...
            case 0x24: {     /*INC IXh*/
                regIXY.setHigh(inc8(regIXY.getHigh()));
                break;
            }
            case 0x25: {     /*DEC IXh*/
                regIXY.setHigh(dec8(regIXY.getHigh()));
                break;
            }
            case 0x26: {     /*LD IXh,n*/
                regIXY.setHigh(MemIoImpl.peek8(regPC));
                regPC = ++regPC & 0xffff;
                break;
            }
            case 0x29: {     /*ADD IX,IX*/
                regIXY.setWord(add16(regIXY.getWord(), regIXY.getWord()));
                tEstados += MemIoImpl.MREQstates((regI<<8)|getRegR(), 7);
                break;
            }
            case 0x2A: {     /*LD IX,(nn)*/
                work16 = MemIoImpl.peek16(regPC);
                regIXY.setWord(MemIoImpl.peek16(work16));
                regPC = ((regPC + 2) & 0xffff);
                break;
            }
            case 0x2B: {     /*DEC IX*/
            ...
            case 0x2C: {     /*INC IXl*/
                regIXY.setLow(inc8(regIXY.getLow()));
                break;
            }
            case 0x2D: {     /*DEC IXl*/
                regIXY.setLow(dec8(regIXY.getLow()));
                break;
            }
            case 0x2E: {     /*LD IXl,n*/
                regIXY.setLow(MemIoImpl.peek8(regPC));
                regPC = ++regPC & 0xffff;
                break;
            }
            case 0x34:       /*INC (IX+d)*/
                offset = (byte) MemIoImpl.peek8(regPC);
                address = regIXY.getWord() + offset;
                tEstados += MemIoImpl.MREQstates(regPC, 5);
            ...
            case 0x39: {     /*ADD IX,SP*/
                regIXY.setWord(add16(regIXY.getWord(), regSP));
                tEstados += MemIoImpl.MREQstates((regI<<8)|getRegR(), 7);
                break;
            }
            case 0x44: {     /*LD B,IXh*/
            ...
            case 0xE3: {     /*EX (SP),IX*/
                ...
                work16 = regIXY.getWord();
                regIXY.setWord(MemIoImpl.peek16(regSP));
                tEstados += MemIoImpl.MREQstates(regSP + 1, 1);
                MemIoImpl.poke8(regSP + 1, (work16 >>> 8));
            ...
            case 0xE9: {     /*JP (IX)*/
                regPC = regIXY.getWord();
                break;
            }
            case 0xF9: {     /*LD SP,IX*/
                regSP = regIXY.getWord();
                tEstados += MemIoImpl.MREQstates(regPC, 2);
                break;
            }
            default: {
    ...
    private final void decodeDDFDCB(IndexRegister regIXY) {
        int work8;
        byte offset = (byte) MemIoImpl.peek8(regPC);
        int address = regIXY.getWord() + offset;
        regPC++;
    ...
    private final void decodeED() {
        ...
        switch (opCode) {
            ...
            case 0x43: {     /*LD (nn),BC*/
                work16 = MemIoImpl.peek16(regPC);
                MemIoImpl.poke16(work16, getRegBC());
                regPC += 2;
                break;
            }
            case 0x44:
            ...
            case 0x4B: {     /*LD BC,(nn)*/
                work16 = MemIoImpl.peek16(regPC);
                setRegBC(MemIoImpl.peek16(work16));
                regPC += 2;
                break;
            }
            case 0x4D: {     /*RETI*/
            ...
            case 0x53: {     /*LD (nn),DE*/
                work16 = MemIoImpl.peek16(regPC);
                MemIoImpl.poke16(work16, getRegDE());
                regPC += 2;
                break;
            }
            case 0x56:
            ...
            case 0x5B: {     /*LD DE,(nn)*/
                work16 = MemIoImpl.peek16(regPC);
                setRegDE(MemIoImpl.peek16(work16));
                regPC += 2;
                break;
            }
            case 0x5E:
            ...
            case 0x63: {     /*LD (nn),HL*/
                work16 = MemIoImpl.peek16(regPC);
                MemIoImpl.poke16(work16, getRegHL());
                regPC += 2;
                break;
            }
            case 0x67: {     /*RRD*/
            ...
            case 0x6B: {     /*LD HL,(nn)*/
                work16 = MemIoImpl.peek16(regPC);
                setRegHL(MemIoImpl.peek16(work16));
                regPC += 2;
                break;
            }
            case 0x6F: {     /*RLD*/
